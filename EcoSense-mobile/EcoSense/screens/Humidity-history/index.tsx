...
const Humidityhistory =()=>{
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const response = await fetch('http://192.168.0.100:8000/data/get_sensor_readings/Humidity',{headers});
    ...
    const fetchMinMaxData = async () => {
      const token = await AsyncStorage.getItem('token');
      const headers = {'Authorization': `Bearer ${token}`};
      const minMax = await fetch('http://192.168.0.100:8000/data/get/Humidity/min_max',{headers});
